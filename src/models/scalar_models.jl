...
function LPData(A, c::VT;
  ...
  return ModelData{eltype(c), VT, Nothing, Nothing, typeof(A_op)}(
    A_op, nothing, lcon, ucon, lvar, uvar, c, _wrap_c0(c, c0), nothing)
end
...
function ScalarModel(data::ModelData{T, VT};
                     x0::VT = fill!(similar(data.c), zero(T)),
                     y0::VT = fill!(similar(data.c, size(data.A, 1)), zero(T)),
                     minimize::Bool = true,
                     name::String = data.Q === nothing ? "LinearModel" : "QuadraticModel") where {T, VT}
...
NLPModels.jac_structure!(m::ScalarModel, rows::AbstractVector{<:Integer}, cols::AbstractVector{<:Integer}) =
...
function NLPModels.jprod!(m::ScalarModel, x::AbstractVector, v::AbstractVector, jv::AbstractVector)
  NLPModels.increment!(m, :neval_jprod)
  mul!(jv, m.data.A, v)
  return jv
end

function NLPModels.jtprod!(m::ScalarModel, x::AbstractVector, v::AbstractVector, jtv::AbstractVector)
