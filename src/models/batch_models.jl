struct BatchModel{T,MT<:AbstractMatrix{T},AOp,QOp,CT,C0T,HX} <:
...
const ObjRHSBatchQuadraticModel{
    T,
    MT,
...
const UniformBatchQuadraticModel{
    T,
    MT,
...
function _batch_meta(
    ::Type{T},
    ::Type{MT},
    meta,
    nbatch;
    x0,
    y0 = fill!(MT(undef, meta.ncon, nbatch), zero(T)),
    lvar,
    uvar,
    lcon,
    ucon,
...
function _stacked_batch(models::Vector, shared_A, shared_c, shared_c0, name, MT)
    ...
    shared_A =
        shared_A === nothing ? _all_equal(models, m -> _sparse_values(m.data.A)) : shared_A
    shared_c = shared_c === nothing ? _all_equal(models, m -> m.data.c) : shared_c
    shared_c0 =
        shared_c0 === nothing ? _all_equal(models, m -> @inbounds m.data.c0[1]) : shared_c0

    c_batch =
        shared_c ? copyto!(similar(MT(undef, 0, 0), T, m1.meta.nvar), m1.data.c) :
        _stack_columns(MT, models, m -> m.data.c)
    c0_batch =
        shared_c0 ? (@inbounds m1.data.c0[1]) : T[@inbounds(m.data.c0[1]) for m in models]

    A =
        shared_A ? sparse_operator(m1.data.A) :
...
function batch_model(
    ...
    m1, MT, meta, c_batch, c0_batch, A =
        _stacked_batch(models, shared_A, shared_c, shared_c0, name, MT)
    Q, HX = if m1 isa QuadraticModel
        shared_Q =
            shared_Q === nothing ? _all_equal(models, m -> _sparse_values(m.data.Q)) :
            shared_Q
        Qop =
            shared_Q ? sparse_operator(m1.data.Q; symmetric = true) :
