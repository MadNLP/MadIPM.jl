...
_qp_mixed() = QuadraticModel(
    ...
    lcon = [1.0, 0.5],
    ucon = [2.0, 1.5],
    lvar = [0.0, -Inf, 0.0, -Inf],
    uvar = [Inf, 5.0, 5.0, Inf],
    x0 = ones(4),
)

_qp_dense() = QuadraticModel(
    [1.0, -1.0],
    [1, 2, 2],
    [1, 1, 2],
    [4.0, 2.0, 3.0];
...
_solve(qp; kw...) = MadIPM.solve!(
    MadIPM.MPCSolver(qp; print_level = MadNLP.ERROR, rethrow_error = true, kw...),
)
_batch(qps; kw...) = MadIPM.madipm_batch(
    ObjRHSBatchQuadraticModel(qps);
    print_level = MadNLP.ERROR,
    rethrow_error = true,
    kw...,
)
_fbatch(qps; kw...) = MadIPM.madipm_batch(
    BatchQuadraticModel(qps);
    print_level = MadNLP.ERROR,
    rethrow_error = true,
    kw...,
)
...
function _init_solver(solver)
    ...
    MadNLP.initialize!(
        ...
        solver.xl,
        solver.xu,
        MadNLP.full(solver.y),
        MadNLP.full(solver.rhs),
        bcb.ind_ineq,
        ws.bx;
...
function _build_bat(qp; kwargs...)
    _init_solver(
        MadIPM.UniformBatchMPCSolver(
            ObjRHSBatchQuadraticModel([qp]);
            print_level = MadNLP.ERROR,
            kwargs...,
        ),
    )
...
function _build_bat_n(qp, n::Int; kwargs...)
    _init_solver(
        MadIPM.UniformBatchMPCSolver(
            ObjRHSBatchQuadraticModel([qp for _ = 1:n]);
            print_level = MadNLP.ERROR,
            kwargs...,
        ),
    )
...
@testset "Batch solver" begin
    ...
    @testset "e2e MehrotraAdaptiveStep: $name" for (name, fn) in
                                                   [PROBLEMS[1], PROBLEMS[3], PROBLEMS[6]]
        qp = fn()
    ...
    @testset "kernel parity: $name" for (name, fn) in PROBLEMS_3
        ...
        @test cmp(SparseArrays.nonzeros(seq.kkt.aug_com), bat.kkt.aug_com_nzvals[:, 1]) <
              1e-12
    ...
    @testset "regularization" begin
        ...
        for (reg1, reg2) in [
            ...
            bat = MadIPM.UniformBatchMPCSolver(
                ObjRHSBatchQuadraticModel([qp]);
                print_level = MadNLP.ERROR,
                regularization = reg2,
            )
    ...
    @testset "bs=3 consistency: $name" for (name, fn) in PROBLEMS_3
        ...
        @test bat3.workspace.alpha_p[1] ==
              bat3.workspace.alpha_p[2] ==
              bat3.workspace.alpha_p[3]
    end
    ...
    @testset "residual INTERNAL_ERROR (all fail)" begin
        stats = _batch([_lp() for _ = 1:3]; check_residual = true, tol_linear_solve = 0.0)
        for i = 1:3
    ...
    @testset "factorize retry" begin
        function _mk(; fail_positions = Set{Int}(), fail_remaining = 0)
            ...
            s = MadIPM.UniformBatchMPCSolver(
                bnlp;
                print_level = MadNLP.ERROR,
        ...
        @testset "identity view" begin
            ...
            MadIPM.factorize_system!(bad)
            @test all(MadIPM.is_factorized(bad.kkt.batch_solver.solvers[j]) for j = 1:3)
    ...
    @testset "_adjust_boundary_active!" begin
        ...
        MadIPM._adjust_boundary_active!(
            ...
            MadIPM.upper(bat.x),
            MadIPM.upper(bat.xu),
            bat.workspace.mu_batch,
            bat.workspace.active_mask,
        )
        ...
        MadIPM._adjust_boundary_active!(
            MadIPM.lower(bat2.x),
            MadIPM.lower(bat2.xl),
            MadIPM.upper(bat2.x),
            MadIPM.upper(bat2.xu),
            bat2.workspace.mu_batch,
            bat2.workspace.active_mask,
        )
    ...
    @testset "partial active KKT solve preserves rhs" begin
        solver = MadIPM.UniformBatchMPCSolver(
            BatchQuadraticModel([_lp() for _ = 1:3]);
            print_level = MadNLP.ERROR,
        )
    ...
    @testset "BatchLinearModel shared A" begin
        lps = [_lin([1.0, 1.0 + 0.1i], [1.0, 1.0]) for i = 1:3]
    ...
    @testset "BatchLinearModel per-instance A" begin
        lps = [_lin([1.0, 1.0], [1.0, 1.0 + 0.2i]) for i = 1:3]
