...
@testset "Batch solver (CUDA)" begin
    ...
    @testset "ObjRHSBatch LP bs=4" begin
        _gpu_batch([_lp() for _ = 1:4]; atol = 1e-5)
    end

    @testset "ObjRHSBatch QP bs=3" begin
        _gpu_batch([_qp() for _ = 1:3])
    end

    @testset "ObjRHSBatch QP doubly bs=2" begin
        _gpu_batch([_qp_db() for _ = 1:2])
    end

    @testset "ObjRHSBatch QP dense bs=2" begin
        _gpu_batch([_qp_dense() for _ = 1:2])
    end
    ...
    @testset "bs=8" begin
        _gpu_batch([_qp() for _ = 1:8])
    end

    @testset "FullBatch identical QP bs=3" begin
        _gpu_batch([_qp() for _ = 1:3]; Model = BatchQuadraticModel)
    end
    ...
    @testset "MehrotraAdaptiveStep" begin
        _gpu_batch([_qp() for _ = 1:3]; step_rule = MadIPM.MehrotraAdaptiveStep(0.99))
    end

    @testset "residual check INTERNAL_ERROR" begin
        cpu_bnlp = ObjRHSBatchQuadraticModel([_lp() for _ = 1:3])
        gpu_bnlp = Adapt.adapt(CuArray, cpu_bnlp)
        stats = MadIPM.madipm_batch(
            gpu_bnlp;
            print_level = MadNLP.ERROR,
